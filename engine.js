...
function loseRace(){ if(typeof endDemo==='function'&&G&&G.demo){endDemo();return;} if(G.over)return; G.over=true; sfx('lose'); addShake(10); addFlash('255,55,55',0.6); showEnd(false,{name:'__race__'}); }
function lose(h){if(typeof endDemo==='function'&&G&&G.demo){endDemo();return;} if(G.over)return;G.over=true;sfx('lose');addShake(13);addFlash('255,55,55',0.7);showEnd(false,h);}
...
/* ---------- CRT overlay — scanlines + corner vignette, toggled by the client ---------- */
let CRT_ON=false,_crtPat=null,_crtVig=null,_crtW=0,_crtH=0;
function applyCrt(){
  if(!CRT_ON)return;
  try{
    if(!_crtPat){
      ...
      g.fillStyle='rgba(0,0,0,0.16)'; g.fillRect(0,2,4,1);   // one dark line per 3px
      _crtPat=ctx.createPattern(c,'repeat');
    }
    if(!_crtVig||_crtW!==vw||_crtH!==vh){
      _crtW=vw;_crtH=vh;
      const v=document.createElement('canvas'); v.width=vw; v.height=vh;
      const g=v.getContext('2d');
      const rg=g.createRadialGradient(vw/2,vh/2,Math.min(vw,vh)*0.55,vw/2,vh/2,Math.max(vw,vh)*0.78);
      rg.addColorStop(0,'rgba(0,0,0,0)'); rg.addColorStop(1,'rgba(4,2,8,0.42)');
      g.fillStyle=rg; g.fillRect(0,0,vw,vh);
      _crtVig=v;
    }
    ctx.setTransform(1,0,0,1,0,0);
    ctx.fillStyle=_crtPat; ctx.fillRect(0,0,vw,vh);
    ctx.drawImage(_crtVig,0,0);
  }catch(e){}
}
